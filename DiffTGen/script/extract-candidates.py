...
def get_plausible(sim: List[dict]) -> Set[str]:
  plau_list = set()
  for loc in sim:
    res = sim[loc]
    if res["plausible"]:
...
def main_recoder(cmd: str, rootdir: str, outdir: str) -> None:
  ...
  if cmd == "recoder":
    ...
    tool_dir = os.path.join(rootdir, "SelfAPR")
  elif cmd == "srepair":
    tool_dir = os.path.join(rootdir, "SRepair")
  elif cmd == "tbar":
    ...
    with open(switch_info_file, "r") as swf, open(sim_file, "r") as sf:
      ...
      for file_info in sw["rules"]:
        ...
        for func_info in file_info["functions"]:  
          for line_info in func_info["lines"]:
            for case_info in line_info["cases"]:
              loc = case_info["location"]
              if loc in plau_list:
                patch_info = {"id": loc.replace("/", "_"), "location": loc, "file": file_name, "line": line_info["line"]}
                plau_patches.append(patch_info)
      if len(plau_list) == 0:
        continue
      print(f"[done] [bugid {bugid}]")
...
if __name__ == "__main__":
  ...
  # if cmd in ["recoder", "alpharepair", "selfapr", "srepair"]:
  #   main_recoder(cmd, sys.argv[2], sys.argv[3])
