...
def instrument_patched_project(work_dir:str,buggy_project:str,buggy_path:str):
  ...
  classpath=f'{instrumenter_root}/build/libs/ASMFieldChange.jar'
...
def test_patched_project(patch_location: str, buggy_location: str, work_dir: str, test: str, buggy_project: str, class_file: str):
  ...
  try:
    if os.environ['GREYBOX_FIELD']=='1':
      # Copy runtime classes to source directory
      # We copy the runtime classes before the instrumentation is success for the later patches
      src_path=work_dir+get_src_paths(buggy_project)[0]
      shutil.copytree(f'{os.environ["GREYBOX_INSTR_ROOT"]}/src/main/java/kr/ac/unist/apr/runtime',src_path+'/kr/ac/unist/apr/runtime',dirs_exist_ok=True)
      ...
      # if not os.path.exists(src_path+'/kr/ac/unist/apr/runtime'):
      #   copyfile(f'{os.environ["GREYBOX_INSTR_ROOT"]}/src/main/resources/kr/ac/unist/apr/runtime',src_path+'/kr/ac/unist/apr/runtime')
