...
def run(project,mode,seed):
    ...
    if mode=='vertical':
        ...
        result=subprocess.run(['python3',f'{new_cur_dir}/SimAPR/simapr.py','-o',f'result/{project}-wo-vertical','-m','guided','--seed',f'{seed}',"--use-exp-alpha",'--instr-cp','/root/project/JPatchInst',
        ...
        result=subprocess.run(['python3',f'{new_cur_dir}/SimAPR/simapr.py','-o',f'result/{project}-wo-horizontal','-m','guided','--seed',f'{seed}',"--use-exp-alpha",'--instr-cp','/root/project/JPatchInst',
                    '--tbar-mode','-w',f'{new_cur_dir}/Avatar/d4j/{project}','-t','180000','--use-pass-test','--use-simulation-mode',f'result/{project}-cache.json',
