...
def run(project):
   for i in range(10):
      ...
      # result=subprocess.run(['python3','search-recoder-ablation-greybox.py',project,'vertical','branch',str(seeds.SEEDS[i]),str(i)],
      ...
      if os.path.exists(f'result/{project}-greybox-fieldonly-{i}.log'):
         ...
         result=subprocess.run(['python3','search-recoder-greybox-fieldonly.py',project,str(seeds.SEEDS[i]),str(i)],
                              stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
      ...
      if os.path.exists(f'result/{project}-wo-vertical-field-{i}.log'):
         ...
         result=subprocess.run(['python3','search-recoder-ablation-greybox.py',project,'vertical','field',str(seeds.SEEDS[i]),str(i)],
                              stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
      ...
      if os.path.exists(f'result/{project}-wo-vertical-both-{i}.log'):
         ...
         result=subprocess.run(['python3','search-recoder-ablation-greybox.py',project,'vertical','both',str(seeds.SEEDS[i]),str(i)],
                              stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
