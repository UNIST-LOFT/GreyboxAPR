...
def run(project,seed,trial):
    ...
    result=subprocess.run(['python3',f'{new_cur_dir}/SimAPR/simapr.py','-o',f'result/{project}-greybox-{trial}','-m','greybox',
                           ...
                           '--branch-output',f'result/branch/{project}','-E','3000','-T','18000','--skip-valid','--optimized-instrumentation',
...
if __name__ == '__main__':
    args=sys.argv
    if len(args)!=4:
        print('Usage: python3 search-alpharepair-greybox.py <project> <seed> <trial>')
        sys.exit(1)
