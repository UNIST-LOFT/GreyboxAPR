import subprocess
import getopt
import sys
import os

def main(argv):
    if len(argv) < 2:
        print("Usage: python3 selfapr.py <project> [gpu-core]")
        exit(1)

    bugid = argv[1]
    os.chdir("../../SelfAPR")
    os.makedirs("d4j", exist_ok=True)
    print(f"Run SelfAPR: {bugid}")
    cmd = f"conda run -n alpha python3 0_localize_fault.py {bugid}"
    result = subprocess.run(cmd, shell=True, executable='/bin/bash')
    cmd = f"conda run -n alpha python3 1_bug_representation.py {bugid}"
    result = subprocess.run(cmd, shell=True, executable='/bin/bash')
    cmd = f"conda run -n alpha python3 2_test.py {bugid}"
    new_env=os.environ.copy()
    if len(argv) > 2:
        new_env["CUDA_VISIBLE_DEVICES"]=argv[2]
    result=subprocess.run(cmd, env=new_env, shell=True, executable='/bin/bash')
    print(f"SelfAPR finished raw patch gen {bugid} with return code {result.returncode}")
    f"conda run -n alpha python3 3_evaluate_patch.py {bugid}"
    result = subprocess.run(cmd, shell=True, executable='/bin/bash')
    exit(result.returncode)

if __name__ == "__main__":
    main(sys.argv)