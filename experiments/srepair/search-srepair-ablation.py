...
def run(project,mode,seed,trial):
    ...
    if mode=='vertical':
        ...
        result=subprocess.run(['python3',f'{new_cur_dir}/SimAPR/simapr.py','-o',f'result/{project}-wo-vertical-{trial}',
                               ...
                               '-w',f'{new_cur_dir}/SRepair/d4j/{project}','-t','180000',
        ...
        result=subprocess.run(['python3',f'{new_cur_dir}/SimAPR/simapr.py','-o',f'result/{project}-fieldonly-{trial}',
                               '-m','greybox','--seed',f'{seed}','-k','learning','--skip-valid',
                               '-w',f'{new_cur_dir}/SRepair/d4j/{project}','-t','180000',
                               '--use-simulation-mode',f'result/cache/{project}-cache.json','-E','3000','--not-use-epsilon',
                               '--not-use-guide','--use-field',
                               '--','python3',f'{new_cur_dir}/SimAPR/script/d4j_run_test.py',f'{new_cur_dir}/SRepair/buggy'])
...
if __name__ == '__main__':
    args=sys.argv
    if len(args)!=5:
        print('Usage: python3 search-srepair-ablation.py <project> <vertical|horizontal|field> <seed> <trial>')
        sys.exit(1)
