...
def run(project,mode,target,seed,trial):
    ...
    if mode_args is not None and target_args is not None:
        ...
        result=subprocess.run(['python3',f'{new_cur_dir}/SimAPR/simapr.py','-o',f'result/{project}-wo-{mode}-{target}-{trial}',
                            '-m','greybox','--seed',f'{seed}','-k','learning','-w',f'{new_cur_dir}/SRepair/d4j/{project}',
...
if __name__ == '__main__':
    args=sys.argv
    if len(args)!=6:
        print('Usage: python3 search-srepair-ablation-greybox.py <project> <vertical|horizontal> <branch|field|both> <seed> <trial>')
        sys.exit(1)
